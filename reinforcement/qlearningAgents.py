...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        #初始化一个计数器，用于存储(state, action)形式的键，和值
    ...
    def computeValueFromQValues(self, state):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
        #补全代码，取得合法的行为(见qlearningAgents.py文件中class QLearningAgent类中关于Functions you should use的提示)：
        ...
        if len(actions) == 0:
            return 0
        #遍历每个行为，将行为和根据行为计算的Q值追加进allActions中
        else:
            for action in actions:
                #补全代码：获得当前状态行为对的q值并将该q值存入values列表中
                #传入state和action，计算Q值
